(function () {
    ...
    angular.module('ng.ueditor', [])
        .factory('ueditorPromise', ['$q', '$http', function ($q, $http) {
            var config;

            try {
                config = JSON.parse(document.getElementById('ueditor').innerHTML)
            } catch (e) {
                console.warn('配置解析失败');
            }

            return $http.jsonp(config.url.config).finally(function () {
                return $http.jsonp(config.url.main);
            });
        }])
        .directive(
            'ngUeditor',
                ...
                function(ueditorPromise) {
                    return {
                        ...
                        link: function($S, element, attr, ctrl) {
                            ...
                            ueditorPromise.finally(function () {
                                new _NGUeditor();
                            });
                        }
                    };
