...
Webmusic.prototype={
 ...
 // 判断是否 播放下一首
	...
	$('.voide_ct .volume').on('click',function(){       // 点击 静音
	  if($('.voide_ct .frame').contents().find('#player')[0].volume === 0.0){
	    $('.voide_ct  .frame').contents().find('#player')[0].volume=_this.volume;
	    _this.volumeOn();
	     }else{
	     $('.voide_ct .frame').contents().find('#player')[0].volume = 0.0;
	     _this.volumeOff();
	     }
	   });
...
$('.voide_ct .time_outline').on('click',function(e){
				 _this.currentTime=((e.pageX-$(this).offset().left)/200)*$('.voide_ct .frame').contents().find('#player')[0].duration;
				 $('.voide_ct .frame').contents().find('#player')[0].currentTime=_this.currentTime;
...
$('.voide_ct .fa-refresh').on('click',function(){  // 点击 增/减 单曲循环状态
			if(_this.looping){
				$('.voide_ct .frame').contents().find('#player').removeAttr('loop');
				$(this).css('color', '#fff');
				_this.looping=false;
			}else{
				$('.voide_ct .frame').contents().find('#player').attr('loop','');
				$(this).css('color', '#6c9');
				_this.looping=true;
			}
		});
...
appendRadioHtml:function(){
	var _this=this,
			...
			radioHtml+='<div class="music_wrap">'+
									...
									'<iframe  class="frame"src="acorn.html" frameborder="0"></iframe>'+
									'</div>'+
...
//默认选择第一个频道
	...
	//停止播放
	stopPlay:function(){
		$(".voide_ct .frame").contents().find('#player')[0].pause();
...
//获取歌曲总时长时间
		setDuration:function(){
			var _this=this;
			var timeNumber=_this.dealTime($('.voide_ct .frame').contents().find('#player')[0].duration);
...
setTime: function(){
			 ...
			 var clock=setInterval(function(){
				 var time=$('.voide_ct .frame').contents().find('#player')[0].currentTime/$('.voide_ct .frame').contents().find('#player')[0].duration;
				 var timeNumber=_this.dealTime($('.voide_ct .frame').contents().find('#player')[0].currentTime);
				 $('.voide_ct .cur_time').text(timeNumber);
				 if(time>=1){
...
dealTime: function(data){
	...
	getDouBanSong: function(channel_id){
			...
			$.get('http://api.jirengu.com/fm/getSong.php',{channel: channel_id}, function(ret){
				var ret=JSON.parse(ret);
				$('.voide_ct .frame').contents().find('#player').attr('src', ret.song[0].url);
        ...
        _this.lyricShowing();
			...
			lyricShowing: function(){
				var _this=this;
			 var curTime=$('.voide_ct .frame').contents().find('#player')[0].currentTime;
			 if(!_this.timeArr){
				 return;
			 }
